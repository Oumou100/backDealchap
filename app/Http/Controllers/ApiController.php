...
//                UserFcmToken::insertOrIgnore(['user_id' => $auth->id, 'fcm_token' => $request->fcm_id, 'created_at' => Carbon::now(), 'updated_at' => Carbon::now()]);
                UserFcmToken::updateOrCreate(['fcm_token' => $request->fcm_id,'platform_type' => $request->platform_type], ['user_id' => $auth->id, 'created_at' => Carbon::now(), 'updated_at' => Carbon::now()]);
            }
    ...
    public function addItem(Request $request) {
        ...
        // Vérifier si le slug existe déjà, sinon ajouter un suffixe pour garantir l'unicité
        $slugExist = Item::where('slug', $slug)->first();
        if ($slugExist) {
            $slug = $slug . '-' . time();  // Ajouter un suffixe basé sur le timestamp pour garantir l'unicité
        }

            $data = [
                ...$request->all(),
                'name'        => strtoupper($request->name), // Store name in uppercase
                'slug'        => $slug,  // Ajouter le slug généré ici
                'status'      => "approved", //review,approve,reject
            ...
            $result = Item::with(
                'user:id,name,email,mobile,profile',
    ...
    public function getItem(Request $request) {
        ...
        try {
            ...
            $sql = Item::with('user:id,name,email,mobile,profile,created_at,is_verified,show_personal_details', 'category:id,name,image', 'gallery_images:id,image,item_id', 'featured_items', 'favourites', 'item_custom_field_values.custom_field', 'area:id,name')
    ...
    public function updateItem(Request $request) {
        ...
        try {
            ...
            $data = $request->all();
            $data['slug'] = $uniqueSlug; // Ensure the unique slug is added to the data
...
//                        PaymentTransaction::find($data->id)->update(['payment_status' => "failed"]);
    ...
    public function createItemOffer(Request $request) {
        ...
        try {
            ...
            $notification = NotificationService::sendFcmNotification($user_token, 'New Offer', $message,"offer");
            }
    ...
    public function getSeller(Request $request) {
        // Validate that the 'id' field is required
        $request->validate([
            'id' => 'required|integer'
        ]);
    ...
    public function renewItem(Request $request) {
        try {
            ...
            $package = Package::where('id', $request->package_id)->whereNot('duration','unlimited')->firstOrFail();
